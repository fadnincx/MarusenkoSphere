...
public class Solver {
    ...
    */
	...
	/**
	 * Fügt eine Kugel zum Lösungsprozess hinzu
	 * @return
	 */
	...
	/**
	 * Fügt i Kugeln zum Lösungsprozess hinzu
	 * @param i
	 * @return
	 */
	 ...
	 * @return : gibt die gelöste Kugel zuröck
	 */
	public ArrayList<String> solve(Kugel kugel){
		//Nimmt die aktuelle Kugel zum lösen
		this.k = kugel;
		//Füge die Ausgangskugel als erste Kugel hinzu
		solvingWay.add(k.getSphere());
		
		//Array "ok" mit false f�llen
		...
		while(!SolveCheck.ArrayIsFullyOk(ok)){
			...
			for(int i = 0; i<24; i++){
				...
				//Wenn Position noch nicht gel�st
				if(!ok[i]){
					//Recheck gefordert
					recheck = true;
		 ...
		 */
		consLog("Phase 2");
		//Array ok erneut zur�ck auf false setzen
		Arrays.fill(ok, false); 
		
		//Solange Kugel nicht gel�sst
		while(!SolveCheck.isKugelSolved(k)){
			//Gehe Jeden Pol durch
			for(int i = 0; i<6; i++){
				//Wenn Pol noch nicht gel�sst, l�se ihn
	...
	private void checkIfP1IsOK(){
		...
		for(int i = 0; i<6; i++){
			...
			//Array mit daten aus Kugel f�llen
			for(int j = 0; j<4; j++){	 
	...
	private int findPos(int p){
		...
		for(int i = 0; i<3; i++){
			...
			if(!checkPoleEndPhase1(polNr)&&urPol!=polNr){
				 ...
				 */
				consLog(polNr+" Pol nicht gel�sst");
				for(int j = 0; j<4; j++){
					/**
	...
	/**
	 * Macht die Wechselfunktion der Pole durch
